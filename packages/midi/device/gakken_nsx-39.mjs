// Gakken(YAMAHA) NSX-39 Device Implementation
//import { sysex } from '../midi.mjs';
import { Pattern } from '@strudel/core';

export const nsx39 = {
  // YAMAHA"s manufacturer ID
  id: 0x43,
  // Miku phoneme lookup table
  miku: {
  ...
  // Method to convert phrase to sysex messages
  say(phrase) {
    const messages = [];
    for (let i = 0; i < phrase.length; i++) {
      let char = phrase[i];
      if (i + 1 < phrase.length && this.miku[char + phrase[i + 1]]) {
        char += phrase[i + 1];
        i++;
      }
      if (!this.miku[char]) {
      ...
      messages.push(this.miku[char].trim());
    }
    // Join all messages with a space
    const concatenatedMessage = messages.join(' ');
    return sysex([this.id, Pattern.pure(concatenatedMessage)]);
  },
};
